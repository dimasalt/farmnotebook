...
BEGIN
	...
	(
		...
		WHERE  (transaction_item.item_category <> 'Property' AND transaction_item.item_category <> 'Farm' AND (transaction_item.item_category <> 'vehicle' AND  transaction_item.item_subcategory <> 'Gasoline and oil'))
	...
	(
		...
		WHERE transaction_item.is_expence = 1 
			AND  (transaction_item.item_category <> 'Property' AND transaction_item.item_category <> 'Farm' AND (transaction_item.item_category <> 'vehicle' AND  transaction_item.item_subcategory <> 'Gasoline and oil'))
		GROUP BY Month(transaction.trans_date), YEAR(transaction.trans_date)
		ORDER BY transaction.trans_date DESC
	);
