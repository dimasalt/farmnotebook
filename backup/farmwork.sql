...
INSERT INTO `transaction` (`id`, `trans_desc`, `vendor_name`, `vendor_address`, `trans_currency`, `trans_image`, `trans_date`, `created_at`, `updated_at`) VALUES
	...
	('0c2d36ad-cd81-11ed-8dd1-d8cb8ac0caec', '', 'A-Mar Meats', 'Red Deer Rd E, Val Gagné, ON P0K 1W0', 'C$', NULL, '2022-12-23 00:00:00', '2023-03-28 11:56:00', '2023-03-28 11:56:00'),
	...
	('3d1affb5-cd81-11ed-8dd1-d8cb8ac0caec', '', 'Meat sales to customers', '', 'C$', NULL, '2022-11-17 00:00:00', '2023-03-28 11:57:22', '2023-03-28 11:57:22'),
	...
	('c531e7f3-cd80-11ed-8dd1-d8cb8ac0caec', '', 'A-Mar Meats', 'Red Deer Rd E, Val Gagné, ON P0K 1W0', 'C$', NULL, '2022-11-10 00:00:00', '2023-03-28 11:54:01', '2023-03-28 11:54:01'),
...
INSERT INTO `transaction_category` (`id`, `parent_id`, `category_name`, `category_description`, `created_at`) VALUES
	...
	(61, 26, 'Meat Processing', 'Batcher fees etc...', '2023-03-28 11:59:38'),
	(62, 26, 'Beef', '', '2023-03-29 21:25:05');
...
INSERT INTO `transaction_item` (`id`, `transaction_id`, `item_name`, `item_desc`, `item_category`, `item_subcategory`, `amount`, `hst_tax`, `gst_tax`, `pst_tax`, `is_expence`, `created_at`) VALUES
	...
	('1dffa189-cd81-11ed-8dd1-d8cb8ac0caec', '0c2d36ad-cd81-11ed-8dd1-d8cb8ac0caec', 'Meat processing 1beef', '', 'Other', 'Meat Processing', -356.99, 0.00, 0.00, 0.00, 1, '2023-03-28 11:56:30'),
	...
	('4da1b2c7-cd81-11ed-8dd1-d8cb8ac0caec', '3d1affb5-cd81-11ed-8dd1-d8cb8ac0caec', 'Meat sales, 6 packages of 25lbs each $200', '', 'Other', 'Beef', 1200.00, 0.00, 0.00, 0.00, 0, '2023-03-28 11:57:50'),
	...
	('da5e1f6b-cd80-11ed-8dd1-d8cb8ac0caec', 'c531e7f3-cd80-11ed-8dd1-d8cb8ac0caec', 'Meat processing 1beef', '', 'Other', 'Meat Processing', -345.60, 0.00, 0.00, 0.00, 1, '2023-03-28 11:54:37'),
...
-- Dumping structure for procedure farmwork.feedGetRequirementsWeight
DROP PROCEDURE IF EXISTS `feedGetRequirementsWeight`;
DELIMITER //
...
BEGIN
	...
	DECLARE report_date VARCHAR(50);
 	SET report_date =  CONCAT(report_year, '-', report_month, '-01');
 	
-- 	SELECT report_date;
 	

	-- prepare category term
	...
	-- prepare subcategory term
 	IF LENGTH(subcategory) = 0 THEN
 		SET subcategory = "%";
 	ELSE
 		SET subcategory = CONCAT('%', subcategory, '%') ;
 	END IF;
	...
	SELECT 
		...
		DATE_FORMAT(report_date, '%M, %Y') AS 'trans_date_formated',
		COALESCE(SUM(transaction_item.amount), 0) AS 'monthly_total'
	FROM transaction 
			LEFT JOIN transaction_item	ON transaction_item.transaction_id = transaction.id
 	WHERE  (transaction_item.item_category LIKE category AND transaction_item.item_subcategory LIKE subcategory) 
			AND (transaction_item.item_category <> 'Vehicle' AND transaction_item.item_subcategory <> 'Gasoline and oil')
			AND ( MONTH(transaction.trans_date) = report_month and YEAR(transaction.trans_date) = report_year);		
	
END//
DELIMITER ;

-- Dumping structure for procedure farmwork.reportsGetMonthlyFuel
DROP PROCEDURE IF EXISTS `reportsGetMonthlyFuel`;
DELIMITER //
CREATE PROCEDURE `reportsGetMonthlyFuel`(
	IN `report_month` VARCHAR(50),
	IN `report_year` VARCHAR(50)
)
BEGIN

	-- monthly totals for fuel
	-- drop temporary table if exists
	SELECT 
		transaction.trans_date,
		DATE_FORMAT(transaction.trans_date, '%M %Y') AS 'trans_date_formated',
		SUM(transaction_item.amount) AS 'monthly_total'
	FROM 
		transaction 
	LEFT JOIN transaction_item	ON transaction_item.transaction_id = transaction.id
   WHERE (transaction_item.item_category = 'Vehicle' AND transaction_item.item_subcategory = 'Gasoline and oil')
		AND ( MONTH(transaction.trans_date) = report_month and YEAR(transaction.trans_date) = report_year)
...
-- Dumping structure for procedure farmwork.reportsGetMonthly_new
DROP PROCEDURE IF EXISTS `reportsGetMonthly_new`;
DELIMITER //
CREATE PROCEDURE `reportsGetMonthly_new`(
	IN `report_month` VARCHAR(50),
	IN `report_year` VARCHAR(50),
	IN `category` VARCHAR(50),
	IN `subcategory` VARCHAR(50)
)
    COMMENT 'pulls monthly totals for selected date range. Doesn''t include fuel'
BEGIN
	...
	(
		SELECT transaction.trans_date,
			DATE_FORMAT(report_date, '%M, %Y') AS 'trans_date_formated',
			COALESCE(SUM(transaction_item.amount), 0) AS 'expences_total'
		FROM transaction 
				LEFT JOIN transaction_item	ON transaction_item.transaction_id = transaction.id
	 	WHERE  (transaction_item.item_category LIKE category AND transaction_item.item_subcategory LIKE subcategory) 
				AND (transaction_item.item_category <> 'Vehicle' AND transaction_item.item_subcategory <> 'Gasoline and oil')
				AND ( MONTH(transaction.trans_date) = report_month and YEAR(transaction.trans_date) = report_year)
				AND transaction_item.is_expence = 1
	);
	...
	(
		SELECT transaction.trans_date,
			DATE_FORMAT(report_date, '%M, %Y') AS 'trans_date_formated',
			COALESCE(SUM(transaction_item.amount), 0) AS 'income_total'
		FROM transaction 
				LEFT JOIN transaction_item	ON transaction_item.transaction_id = transaction.id
	 	WHERE  (transaction_item.item_category LIKE category AND transaction_item.item_subcategory LIKE subcategory) 
				AND (transaction_item.item_category <> 'Vehicle' AND transaction_item.item_subcategory <> 'Gasoline and oil')
				AND ( MONTH(transaction.trans_date) = report_month and YEAR(transaction.trans_date) = report_year)
				AND transaction_item.is_expence = 0
	);
	
	
	SELECT 
		reports_monthly_total.trans_date_formated,
	...
	LEFT JOIN reports_monthly_expences ON reports_monthly_expences.trans_date_formated = reports_monthly_total.trans_date_formated
	LEFT JOIN reports_monthly_income ON reports_monthly_income.trans_date_formated = reports_monthly_expences.trans_date_formated;
	-- WHERE reports_monthly_total.trans_date >= start_date AND reports_monthly_total.trans_date <= end_date
	-- ORDER BY reports_monthly_total.trans_date DESC;
	...
	IF LENGTH(category_selected) = 0 THEN
		...
		(
			SELECT 
				...
				DATE_FORMAT(transaction.trans_date, '%d-%b-%Y') AS trans_read_date
			FROM 
				transaction
			WHERE 		
				(transaction.vendor_name LIKE search_term
